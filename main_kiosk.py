...
# 조건
# 사용자는 최대로 버거 1개, 사이드 1개, 음료 1개를 주문할 수 있다.
...
if menu_num == 1:  # 햄버거 세트
    ...
    while True:
        menu_choice = int(input('>>번호:'))
        if menu_choice >=1 and menu_choice <=3:
            menu_save['burger'] = burger_name[menu_choice]
            price_save['burger'] = burger_price[menu_choice]
            break
        else:
            print('# MSG: 1~3의 번호를 입력해주세요')
    ...
    while True:
        side_choice = int(input('>>번호:'))
        if side_choice >= 1 and side_choice <= 2:
            menu_save['side'] = side_name[side_choice]
            price_save['side'] = side_price[side_choice]
            break
        else:
            print('# MSG: 1~2의 번호만 입력해주세요')
